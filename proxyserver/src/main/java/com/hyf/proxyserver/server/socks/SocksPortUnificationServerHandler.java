...
public class SocksPortUnificationServerHandler extends ByteToMessageDecoder {
        ...
        if (in.writerIndex() != readerIndex) {
            ...
            switch (version) {
                case SOCKS4a:
                    ...
                    p.addAfter(ctx.name(), Socks4ServerEncoder.class.getName(), Socks4ServerEncoder.INSTANCE);
                    p.addAfter(ctx.name(), Socks4ServerDecoder.class.getName(), new Socks4ServerDecoder());
                    break;
                case SOCKS5:
                    logKnownVersion(ctx, version);
                    p.addAfter(ctx.name(), Socks5ServerEncoder.class.getName(), this.socks5encoder);
                    p.addAfter(ctx.name(), Socks5InitialRequestDecoder.class.getName(), new Socks5InitialRequestDecoder());
                    break;
                default:
                    // 交给后面的处理
                    handleUnknownProtocol(ctx, in, out);
    ...
    protected void handleUnknownProtocol(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
    }
